...
// Quote TTL is the single biggest lever on upstream volume. At 20s one open tab
// cost ~54 calls/min against a 60/min ceiling; at 60s it costs ~18. The page
// polls on the same interval, so a longer TTL means fewer calls, not stale ones.
...
const routes = {
  async quote(q) {
    ...
    await Promise.all(symbols.map(async (s) => {
      try {
        const d = await cached("q:" + s, QUOTE_TTL, () => finnhub("/quote?symbol=" + s));
        out[s] = d && d.c
    ...
    const items = await Promise.all(symbols.map(async (s) => {
      try {
        const [quote, profile] = await Promise.all([
          cached("q:" + s, QUOTE_TTL, () => finnhub("/quote?symbol=" + s)),
  ...
  async candles(q) {
    ...
    return {
      ...
      // Price fields so the chart can render any symbol without it having been
      // polled first — this is what lets Movers rows be clickable.
      last: live.price,
      pct: live.pct,
      change: live.change,
